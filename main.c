...
assignedIndexes[1000], len, lenCopy, flag = 0, childCount =
  1, propertyCount = 
0;

//Fetches property numer when its value is provided
  int
propertyNumber (int value)
{
...
if (isAssigned == 0)
			  {	//if it is not assigned, add to assigned array 
...
int start = 0, i, ta = arr4[pos - 1], m = 0, z =
			  pos, sum1 = 0; 
if (flag == 0)
			  {
			  
			  // By this the user get intimated about the child holding
			  printf ("\nProperties given to child %d:",
				  childCount++); 
for (i = 0; i < pos; i++)
			  {
			  
printf ("\nProperty: %d, Value: %d",
				   propertyNumber (arr1[arr4[i]]),
...
return 1; 
			  ...
			  if (n == 0) 
 return 0; 
			  // if current value is greater than sum then we leave that and process from next element 
			  if (set[n - 1] > sum) 
...
b =
			  isSubsetSum (set, n - 1, sum - set[n - 1], arr4,
				       pos + 1); 
...
printf ("Algorithmic Toolbox Mini Project: Fair Property Division:"); 
printf ("\n19BCI0036: Akashdeep");
